...
class JackettIndexer(_PluginBase):
    ...
    def _build_indexer_dict(self, indexer: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Domain 格式：jackett_indexer.<keyword>
        # keyword 从索引器标题提取，只保留 ASCII 字母和数字，小写
        # 例如："M-Team - TP" -> "mteamtp"，"AudioBook Bay" -> "audiobookbay"
        # 如果标题全为非ASCII字符，则 fallback 到 indexer_id
        # 优先从标题提取关键字，非ASCII全标题则 fallback 到 indexer_id
        keyword = re.sub(r'[^a-zA-Z0-9]', '', indexer_title)
        if not keyword:
            keyword = re.sub(r'[^a-zA-Z0-9]', '', indexer_name) or indexer_name
        domain = f"jackett_indexer.{keyword.lower()}"
        ...
        # site_url 必须带 apikey，否则 Jackett 返回错误导致 MoviePilot 连接测试失败
        indexer_dict = {
            "id": f"{self.plugin_name}-{indexer_title}",
            "name": f"{self.plugin_name}-{indexer_title}",
            "url": f"{self._host.rstrip('/')}/api/v2.0/indexers/{indexer_name}/results/torznab/api?apikey={self._api_key}&t=search&q=a&cat=2000,5000&limit=1",
            "domain": domain,
            "public": is_public,
    ...
    def _apply_search_patch(self):
        ...
        def patched_sync(chain_self, keyword, mediainfo=None, sites=None, page=0, area="title", mtype=None):
            results = list(prev_sync(chain_self, keyword, mediainfo, sites, page, area, mtype) or [])
            ...
            extra = plugin_ref._extra_search_sync(chain_self, en_keyword, mediainfo, sites, page, mtype)
            if extra:
                results.extend(extra)
            return results

        async def patched_async(chain_self, keyword, mediainfo=None, sites=None, page=0, area="title", mtype=None):
            results = list(await prev_async(chain_self, keyword, mediainfo, sites, page, area, mtype) or [])
            if not plugin_ref._enabled or not plugin_ref._indexers:
                return results
            if not mediainfo or not keyword or area == "imdbid":
                return results
            ...
            extra = await plugin_ref._extra_search_async(chain_self, en_keyword, mediainfo, sites, page, mtype)
            if extra:
                results.extend(extra)
            return results
    ...
    def _extra_search_sync(self, chain_self, en_keyword: str, mediainfo, sites, page: int, mtype=None) -> list:
    ...
    async def _extra_search_async(self, chain_self, en_keyword: str, mediainfo, sites, page: int, mtype=None) -> list:
    ...
    def sign_in(self, site: Dict[str, Any], required: bool = True) -> Tuple[bool, str]:
        """
        测试 Jackett 索引站点连接是否正常。
        MoviePilot 站点测试时调用此方法。
        """
        if not self._host or not self._api_key:
            return False, "Jackett 未配置（host 或 api_key 为空）"

        # 从 site 中提取 indexer_name（domain 格式：jackett_indexer.<keyword>）
        domain = site.get("domain", "")
        # domain 如 jackett_indexer.mteamtp，取最后一段作为 indexer_name
        indexer_name = domain.split(".")[-1] if "." in domain else domain
        if not indexer_name:
            return False, f"无法从 domain {domain} 提取索引器名称"

        # 构造带完整参数的测试 URL
        test_url = f"{self._host.rstrip('/')}/api/v2.0/indexers/{indexer_name}/results/torznab/api"
        params = {
            "apikey": self._api_key,
            "t": "search",
            "q": "a",
        ...
        try:
            ...
            response = RequestUtils(proxies=self._proxy).get_res(url=full_url)
            if not response:
                return False, f"无法连接到 Jackett ({test_url})"
            if response.status_code != 200:
                return False, f"Jackett 返回 HTTP {response.status_code}"
            # 检查是否返回 XML 错误
            if "<error" in response.text[:200]:
                return False, f"Jackett API 错误：{response.text[:100]}"
            return True, "连接成功"
        except Exception as e:
