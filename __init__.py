...
class JackettIndexer(_PluginBase):
    ...
    def refresh_torrents(
        ...
        site_name = site.get("name") or site.get("domain") or site.get("id") or ""
    ...
    def search_torrents(
        ...
        # Extract site name
        site_name = site.get("name") or site.get("domain") or site.get("id") or ""
        # 空 site_name 时：MoviePilot 全局搜索场景，改为遍历所有已配置 Jackett 索引器
        if not site_name:
            # 详细调试 _indexers 内容
            import json
            logger.info(f"【{self.plugin_name}】站点名称为空（全局搜索模式），遍历所有 Jackett 索引器")
            if not self._host or not self._api_key:
                logger.warning(f"【{self.plugin_name}】Jackett 未配置，全局搜索返回空")
                return results
            global_results = []
            indexer_snapshot = list(getattr(self, '_indexers', []) or [])
            logger.info(f"【{self.plugin_name}】全局搜索，共 {len(indexer_snapshot)} 个索引器")
            for idx in indexer_snapshot:
                idx_domain = (idx or {}).get("domain", "")
                idx_name = (idx or {}).get("name", "unknown")
                if not idx_domain:
                    continue
                try:
                    search_params = self._build_search_params(keyword=keyword, mtype=mtype, page=page or 0)
                    # 从 domain 提取索引器 API 名称
                    domain_clean = idx_domain.replace("http://", "").replace("https://", "").rstrip("/")
                    idx_api_name = domain_clean.split(".")[-1]
                    xml_content = self._search_jackett_api(idx_api_name, search_params)
                    if not xml_content:
                        continue
                    for torrent in self._parse_torznab_xml(xml_content, idx_name):
                        global_results.append(torrent)
                    logger.info(f"【{self.plugin_name}】索引器 {idx_name} 搜索到结果")
                except Exception as ex:
                    logger.warning(f"【{self.plugin_name}】索引器 {idx_name} 搜索异常: {ex}")
            logger.info(f"【{self.plugin_name}】全局搜索完成，关键词={keyword}，返回 {len(global_results)} 个结果")
        ...
        return [
            {
                ...
                "peers": t.peers,
                "pubdate": t.pubdate,
                # imdbid 已移除：TorrentInfo 不再包含该字段
